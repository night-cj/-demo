'use strict';
import { coerceBoolean } from "../../../../methods/utils/methods";
...
function paddingLeft(context, scope) {
  let level = 0;
  if (!coerceBoolean(scope.row[context.props.levelKey])) {
    let _parentMap = scope.store.states._parentMap;
    let num = 0;
    let parent = _parentMap[scope.row[context.props.treeKey]];
    while (parent) {
      num++;
      if (coerceBoolean(parent[context.props.levelKey])) {
        num += parseInt(parent[context.props.levelKey]);
        break;
      }
      parent = _parentMap[parent[context.props.treeKey]];
    }
    level = num;
  } else {
    level = parseInt(scope.row[context.props.levelKey]);
  }
  return level * parseInt(context.props.indentSize.toString()) + 'px';
}
...
function getIds(children, childKey, treeKey, result = []) {
  if (children) {
    children.forEach(child=>{
      result.push(child[treeKey]);
      getIds(child[childKey], childKey, treeKey, result);
    });
  }
  return result;
}

var descendantsIds = function descendantsIds(id, data, treeKey, row, childKey) {
  return getIds(row[childKey], childKey, treeKey);
};
...
var Colspand = function Colspand(context, scope, data) {
  var _a = context.props,
    ...
    row = scope.row,
    result = [];
  var removeIds = descendantsIds(row[treeKey], data, treeKey, row, childKey);
...
var ExpandRemote = function ExpandRemote(context, scope, data) {
  ...
  var CallBack = function CallBack(children) {
    ...
    if (children && children.length > 0) {
      let _map = children.reduce((map, r)=>{
        map[r[treeKey]] = scope.row;
        return map;
      }, {});
      Object.assign(scope.store.states._parentMap, _map);
...
function doExpand(context, scope) {
  ...
  if (scope.row[childKey]) {
    ...
    console.warn('没有获得子元素数据,请在columnTypeProps属性的tree属性下设置remotePromise以远程获取字数据');
    return;
  }
  // Expand(context, scope, data);
}

function getParentMap(data, childKey, treeKey, parentRow, map = {}) {
  data.forEach(row=>{
    if (parentRow) {
      map[row[treeKey]] = parentRow;
    }
    if (row[childKey]) {
      getParentMap(row[childKey], childKey, treeKey, row, map);
    }
  });
  return map;
}
...
var ElTableInject = /** @class */function () {
  ...
  ElTableInject.prototype.Inject = function (scope, context) {
    if (this.isInjected(scope)) return;
    let { childKey, treeKey } = context.props ;
    ...
    scope.store.states._parentMap = getParentMap(scope.store.states._data, childKey, treeKey);
...
var RenderContext = function RenderContext(h, context, scope) {
  ElTableInjecter.Inject(scope, context);
