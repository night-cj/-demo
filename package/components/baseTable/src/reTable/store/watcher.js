import Vue from 'vue';
import {debounce} from '../../../../../methods/utils/methods';
import merge from '../sourceCode/merge';
import { getKeysMap, getRowIdentity, getColumnById, getColumnByKey, orderBy, toggleRowStatus } from '../sourceCode/util';
...
export default Vue.extend({
  ...
  methods: {
    // todo 重写全选逻辑，防止数据量过大时全选响应时间过长的问题
    toggleAllSelection: debounce(10, function() {
      let states = this.states;
      const data = states.data || [];
      if (data.length === 0) return;
      let selection = this.states.selection;
      ...
      data.forEach((item, index) => {
        if (states.selectable) {
          if (states.selectable.call(null, item, index) && value !== (map.get(item) || false)) {
            selectionChanged = true;
          }
        } else {
          if (value !== (map.get(item) || false)) {
            selectionChanged = true;
          }
        }
      });
      if (value) {
        this.states.selection = selection = data.filter((row, index) => states.selectable ? states.selectable.call(null, row, index) : true);
      } else {
